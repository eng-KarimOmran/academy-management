...
const LessonService: ILessonService = {
  async createLesson({ params, body }) {
    ...
    const lesson = await prisma.$transaction(async (tx) => {
      const { subscription, car, area, jobProfile } = await getValidatedLessonDependencies({ areaId, carId, jobProfileId, subscriptionId, tx, transmission })
  ...
  async getAllLessons({ query, params }) {
    const { academyId } = params;
    const { limit, page, lessonStatus, transmission, search } = query;

    const where = buildLessonWhere({ search, academyId, lessonStatus, transmission });
  ...
  async updateLesson({ body, params }) {
    ...
    const lesson = prisma.$transaction(async (tx) => {
      ...
      const { subscription, car, area, jobProfile } = await getValidatedLessonDependencies({ ...finalData, tx })
