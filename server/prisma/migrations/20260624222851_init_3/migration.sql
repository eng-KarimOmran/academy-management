/*
  Warnings:

  - A unique constraint covering the columns `[name]` on the table `Academy` will be added. If there are existing duplicate values, this will fail.
  - Added the required column `name` to the `Academy` table without a default value. This is not possible if the table is not empty.

*/
-- AlterTable
ALTER TABLE "Academy" ADD COLUMN     "name" TEXT NOT NULL;

-- CreateIndex
CREATE UNIQUE INDEX "Academy_name_key" ON "Academy"("name");
